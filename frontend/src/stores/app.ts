...
export const useAppStore=defineStore('app',{
  ...
  actions:{
    ...
    async bootstrap(){const [settings,types,meta,health]=await Promise.all([api<Settings>('/api/settings'),api<Record<string,ClientType>>('/api/client-types'),api<Record<string,any>>('/api/meta'),api<Record<string,any>>('/api/health')]);Object.assign(this,{settings,types,meta,health});document.title=`${settings.site_name} V${meta.version}`},
    ...
    async logout(){await api('/api/auth/logout',{method:'POST'});this.authenticated=false;this.groups=[]},
  }
})
