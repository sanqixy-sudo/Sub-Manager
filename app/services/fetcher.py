...
class UpstreamFetcher:
    ...
    async def fetch(self, subscription_id: int, upstream: dict[str, Any], pattern: re.Pattern[str]) -> dict[str, Any]:
        ...
        try:
            async with self.client.stream("GET", str(upstream["url"]), headers=headers, timeout=httpx.Timeout(timeout_s, connect=min(15, timeout_s))) as response:
                result["status_code"] = response.status_code
                result["content_type"] = response.headers.get("content-type", "")[:200]
                ...
                async for chunk in response.aiter_bytes():
                    ...
                    if size > MAX_UPSTREAM_BYTES:
                        raise ValueError(f"响应超过 {MAX_UPSTREAM_BYTES // 1024 // 1024} MiB 限制")
                    chunks.append(chunk)
            body = b"".join(chunks)
            ...
            meta_path.write_text(json.dumps({
                "updated_at": utcnow_iso(), "status_code": result["status_code"],
        ...
        update_upstream_result(int(upstream["id"]), result)
        return result
